...
import pandas as pd
import pytest
from aiexec.components.data.kb_ingest import KBIngestionComponent
...
class TestKBIngestionComponent(ComponentTestBaseWithoutClient):
    ...
    def mock_knowledge_base_path(self, tmp_path):
        ...
        with patch("aiexec.components.data.kb_ingest.KNOWLEDGE_BASES_ROOT_PATH", tmp_path):
            yield
    ...
    @patch("aiexec.components.data.kb_ingest.get_settings_service")
    @patch("aiexec.components.data.kb_ingest.encrypt_api_key")
    ...
    def test_convert_df_to_data_objects(self, component_class, default_kwargs):
        ...
        with patch("aiexec.components.data.kb_ingest.Chroma") as mock_chroma:
    ...
    def test_convert_df_to_data_objects_no_duplicates(self, component_class, default_kwargs):
        ...
        with patch("aiexec.components.data.kb_ingest.Chroma") as mock_chroma:
            ...
            with patch("aiexec.components.data.kb_ingest.hashlib.sha256") as mock_hash:
    ...
    @patch("aiexec.components.data.kb_ingest.json.loads")
    @patch("aiexec.components.data.kb_ingest.decrypt_api_key")
    ...
    @patch("aiexec.components.data.kb_ingest.Path.exists")
