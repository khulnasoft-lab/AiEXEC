...
import pytest
from aiexec.custom.utils import _generate_code_hash
...
class TestMetadataInTemplateBuilders:
    ...
    @patch("aiexec.custom.utils.ComponentFrontendNode")
    def test_build_from_inputs_adds_metadata_with_module(self, mock_frontend_class):
        ...
        from aiexec.custom.utils import build_custom_component_template_from_inputs
        ...
        with patch("aiexec.custom.utils.get_component_instance") as mock_get_instance:
            mock_instance = Mock()
            ...
            with (
                patch("aiexec.custom.utils.add_code_field", return_value=mock_frontend),
                patch("aiexec.custom.utils.reorder_fields"),
            ):
                # Call the function
    ...
    @patch("aiexec.custom.utils.CustomComponentFrontendNode")
    def test_build_template_adds_metadata_with_module(self, mock_frontend_class):
        ...
        with patch("aiexec.custom.utils.run_build_config") as mock_run_build:
            mock_instance = Mock()
            mock_instance._get_field_order = Mock(return_value=[])
            mock_run_build.return_value = ({}, mock_instance)

            with (
                patch("aiexec.custom.utils.add_extra_fields"),
                patch("aiexec.custom.utils.add_code_field", return_value=mock_frontend),
                patch("aiexec.custom.utils.add_base_classes"),
                patch("aiexec.custom.utils.add_output_types"),
                patch("aiexec.custom.utils.reorder_fields"),
            ):
                # Call the function
