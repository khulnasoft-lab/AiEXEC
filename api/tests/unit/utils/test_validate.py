...
"""
    ...
    @patch("aiexec.utils.validate.logger")
...
"""
        ...
        with patch("aiexec.utils.validate.prepare_global_scope") as mock_prepare:
            mock_prepare.return_value = {"CustomComponent": type("CustomComponent", (), {})}
            with patch("aiexec.utils.validate.extract_class_code") as mock_extract:
                mock_extract.return_value = Mock()
                with patch("aiexec.utils.validate.compile_class_code") as mock_compile:
                    mock_compile.return_value = compile("pass", "<string>", "exec")
                    with patch("aiexec.utils.validate.build_class_constructor") as mock_build:
...
"""
        ...
        with (
            patch("aiexec.utils.validate.prepare_global_scope", side_effect=validation_error),
...
class TestGetDefaultImports:
    ...
    @patch("aiexec.utils.validate.CUSTOM_COMPONENT_SUPPORTED_TYPES", {"TestType": Mock()})
    ...
    @patch("aiexec.utils.validate.CUSTOM_COMPONENT_SUPPORTED_TYPES", {"CustomType": Mock()})
