...
async def perform_health_checks(context: AiexecContext):
...
async def lifespan(server: FastMCP) -> AsyncIterator[AiexecContext]:
    ...
    with _initialization_lock:
        ...
        try:
            ...
            # Create context
            context = AiexecContext(service_client=service_client)
...
MCP_INSTRUCTIONS = """
# Aiexec MCP Server Instructions

## 🚨 CRITICAL RULES (ALWAYS FOLLOW)
1. **Task Management**: ALWAYS use Aiexec MCP tools for task management.
   - Combine with your local TODO tools for granular tracking
   - First TODO: Update Aiexec task status
   - Last TODO: Update Aiexec with findings/completion
...
try:
    ...
    logger.info("   Server Name: aiexec-mcp-server")
    logger.info("   Description: MCP server using HTTP calls")

    mcp = FastMCP(
        "aiexec-mcp-server",
        description="MCP server for Aiexec - uses HTTP calls to other services",
...
def main():
    ...
    try:
        ...
        setup_logfire(service_name="aiexec-mcp-server")
