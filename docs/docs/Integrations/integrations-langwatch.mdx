...
To integrate with Aiexec, add your LangWatch API key as a Aiexec environment variable:

1. Get a LangWatch API key from your LangWatch account.

2. Add the key to your Aiexec `.env` file:
...
3. Restart Aiexec with your `.env` file, if you modified the Aiexec `.env`:

    ```
    aiexec run --env-file .env
    ```
