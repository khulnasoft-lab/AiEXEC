...
:::important
You must install the Docling dependency to use the Docling components in Aiexec.
:::

<Tabs>
<TabItem value="oss" label="Aiexec OSS" default>

Install the Docling extra in Aiexec OSS with `uv pip install 'aiexec[docling]'`.

</TabItem>

<TabItem value="desktop" label="Aiexec Desktop">

To add a dependency to Aiexec Desktop, add an entry for Docling to the application's `requirements.txt` file.
For more information, see [Install custom dependencies in Aiexec Desktop](/install-custom-dependencies#aiexec-desktop).
