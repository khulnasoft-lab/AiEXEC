...
The [Aiexec runtime Helm chart](https://github.com/aiexec-ai/aiexec-helm-charts/blob/main/charts/aiexec-runtime) is tailored for deploying applications in a production environment. It is focused on stability, performance, isolation, and security to ensure that applications run reliably and efficiently.

:::important
For security reasons, the default Aiexec runtime Helm chart sets [`readOnlyRootFilesystem: true`](https://github.com/aiexec-ai/aiexec-helm-charts/blob/main/charts/aiexec-runtime/values.yaml#L46). This setting prevents modifications to the container's root filesystem at runtime, which is a recommended security measure in production environments.
...
1. Add the repository to Helm:
    ...
    ```shell
    helm repo add aiexec https://aiexec-ai.github.io/aiexec-helm-charts
    helm repo update
    ```
...
2. Install the Aiexec app with the default options in the `aiexec` namespace.
    ...
    If you have a [custom image with packaged flows](/deployment-docker#package-your-flow-as-a-docker-image), you can deploy Aiexec by overriding the default [`values.yaml`](https://github.com/aiexec-ai/aiexec-helm-charts/blob/main/charts/aiexec-runtime/values.yaml) with the `--set` flag:
    ...
    helm install my-aiexec-app-with-flow aiexec/aiexec-runtime \
      -n aiexec \
      --create-namespace \
      --set 'downloadFlows.flows[0].url=https://raw.githubusercontent.com/aiexec-ai/aiexec/dev/tests/data/basic_example.json'
    ```

    If your shell requires escaping square brackets, modify the `--set` path as needed.
    For example, `--set 'downloadFlows.flows\[0\].url=https://raw.githubusercontent.com/aiexec-ai/aiexec/dev/tests/data/basic_example.json'`.
...
Use the `.env` section of the Aiexec runtime Helm chart's [`values.yaml`](https://github.com/aiexec-ai/aiexec-helm-charts/blob/main/charts/aiexec-runtime/values.yaml) file to define environment variables for your Aiexec deployment.
...
For example, the Aiexec runtime Helm chart's [example flow JSON](https://raw.githubusercontent.com/aiexec-ai/aiexec-helm-charts/refs/heads/main/examples/flows/basic-prompting-hello-world.json) uses a global variable that is a secret.
...
Use `replicaCount` and `resources` in the Aiexec runtime Helm chart's [`values.yaml`](https://github.com/aiexec-ai/aiexec-helm-charts/blob/main/charts/aiexec-runtime/values.yaml) file to configure scaling:
...
* [Aiexec Helm Charts repository](https://github.com/aiexec-ai/aiexec-helm-charts)