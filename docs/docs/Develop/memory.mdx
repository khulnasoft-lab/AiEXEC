...
- **OSS macOS/Windows/Linux/WSL (`git clone`)**: `<path_to_clone>/api/base/aiexec/aiexec.db`
...
For more information, see the database models in the [source code](https://github.com/khulnasoft-lab/aiexec/tree/main/api/base/aiexec/services/database/models).
...
Built-in chat memory stores memories in the Aiexec `messages` table.
...
* **Language Model and Agent components**: All messages are stored in [Aiexec storage](#storage-options-and-paths).
...
* **Message History component**: By default, this component stores and retrieves memories from Aiexec storage, unless you attach a **Chat Memory** component. It provides a few more options for sorting and filtering memories, although most of these options are built-in to the **Agent** component as configurable or fixed parameters.
