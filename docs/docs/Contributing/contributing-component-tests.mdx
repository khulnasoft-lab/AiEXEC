...
* The test file should follow the same directory structure as the component being tested, but should be placed in the corresponding unit tests folder.
    ...
    For example, if the file path for the component is `api/base/aiexec/components/prompts/`, then the test file should be located at `api/tests/unit/components/prompts`.
...
To standardize component tests, base test classes have been created and should be imported and inherited by all component test classes. These base classes are located in the file `api/tests/unit/base.py`.
