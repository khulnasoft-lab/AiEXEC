...
Use the `/projects` endpoint to create, read, update, and delete [Aiexec projects](/concepts-flows#projects).

## Read projects

Get a list of Aiexec projects, including project IDs, names, and descriptions.
...
```bash
curl -X GET \
  "$AIEXEC_URL/api/v1/projects/download/$PROJECT_ID" \
  -H "accept: application/json" \
  -H "x-api-key: $AIEXEC_API_KEY" \
...
Import a project and its flows by uploading a Aiexec project zip file:
