...
:::important
The **Language Model** and **Agent** components have built-in chat memory that is enabled by default and uses Aiexec storage.

This built-in chat memory functionality is sufficient for most use cases.

Use the **Message History** component only if you need to access chat memories outside the chat context, such as sentiment analysis flow that retrieves and analyzes recently stored memories, or you want to store memories in a specific database, separate from Aiexec storage.
...
| Name | Type | Description |
|------|------|-------------|
| memory | Memory | Input parameter. Retrieve messages from an external memory. If empty, the Aiexec tables are used. |
...
For example:
    ...
    Variables in the template dynamically add fields to the **Prompt Template** component so that your flow can receive definitions for those values from other components, Aiexec global variables, or fixed input.
