...
The built-in LLMs are appropriate for most text-based embedding model use cases in Aiexec.
...
2. Add an **Embedding Model** component, and then provide a valid OpenAI API key.
    ...
    You can enter component API keys directly or use Aiexec global variables to reference your API keys.
