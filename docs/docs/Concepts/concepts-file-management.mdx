...
Files uploaded to Aiexec file management are stored locally to your Aiexec server, and they are available to all of your flows.
...
1. Navigate to Aiexec file management:
    ...
    * In Aiexec, on the [**Projects** page](/concepts-flows#projects) page, click **My Files** below the list of projects.
    * From a browser, navigate to your Aiexec server's `/files` endpoint, such as `http://localhost:7860/files`. Modify the base URL as needed for your Aiexec server.
    * For programmatic file management, use the [Aiexec API files endpoints](/api-files). However, the following steps assume you're using the file management UI.
...
For videos, see the **Twelve Labs** and **YouTube** [bundles](/components-bundle-components) in the Aiexec **Components** menu.
