...
For information about using Aiexec as an MCP client, see [Use Aiexec as an MCP client](/mcp-client).

As an MCP server, Aiexec exposes your flows as [tools](https://modelcontextprotocol.io/docs/concepts/tools) that [MCP clients](https://modelcontextprotocol.io/clients) can use to take actions.

## Prerequisites

* A [Aiexec project](/concepts-flows#projects) with at least one flow.
...
Each [Aiexec project](/concepts-flows#projects) has an MCP server that exposes the project's flows as tools that MCP clients can use to generate responses.
...
4. Recommended: Edit the **Tool Name** and **Tool Description** to help MCP clients determine which actions your flows provide and when to use those actions:
    ...
    MCP clients like [Cursor](https://www.cursor.com/) treat your Aiexec project as a single MCP server with all of your enabled flows listed as tools.
...
:::important
Auto installation only works if your HTTP client and Aiexec server are on the same local machine.
...
2. In the Aiexec dashboard, select the project that contains the flows you want to serve, and then click the **MCP Server** tab.
3. To auto install your current Aiexec project as an MCP server, click <Icon name="Plus" aria-hidden="True"/> **Add**.
...
For example:
    ...
    If you have [deployed a public Aiexec server](/deployment-public-server), the address is automatically included.
...
To deploy your MCP server externally with ngrok, see [Deploy a public Aiexec server](/deployment-public-server).
